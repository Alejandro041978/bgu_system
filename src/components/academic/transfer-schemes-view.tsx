...
function SchemeDetail({ detail, program, scale, onReload, onDelete }: {
  ...
  const bulkDocs = [...new Set(bulkText.split(/[\s,;]+/).map(s => s.trim()).filter(Boolean))]
  ...
  async function applyBulk() {
    if (bulkDocs.length === 0 || items.length === 0) return
    setBulkApplying(true); setBulkResult(null)
    const res = await fetch(`/api/academic/transfer-schemes/${scheme.id}/apply-bulk`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ documents: bulkDocs }),
    })
    setBulkApplying(false)
    const d = await res.json().catch(() => ({}))
    if (!res.ok) { alert(d.error ?? 'Error'); return }
    const parts = [`Aplicado a ${d.applied} estudiante(s)`]
    if (d.skipped_existing) parts.push(`${d.skipped_existing} ya tenían el esquema`)
    if (d.not_found) parts.push(`${d.not_found} documento(s) no encontrado(s)`)
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        ...
        {/* Carga masiva por lista de documentos */}
        <div className="mt-5 pt-4 border-t border-gray-100">
          <p className="text-xs font-semibold text-gray-700 mb-1">¿Muchos estudiantes? Pega la lista de documentos</p>
          <p className="text-xs text-gray-500 mb-2">Un documento por línea (o separados por coma/espacio). Ideal para el CSV que exportaste de Supabase. No duplica a quien ya tenga el esquema.</p>
          <textarea value={bulkText} onChange={e => setBulkText(e.target.value)} rows={4}
            placeholder={'74958279\n48256392\n74294203\n…'}
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-1 focus:ring-blue-500" />
          <div className="flex items-center gap-3 mt-2">
            <button onClick={applyBulk} disabled={bulkApplying || bulkDocs.length === 0 || items.length === 0}
              className="flex items-center gap-1.5 text-sm font-medium bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 disabled:opacity-50">
              {bulkApplying ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />} Aplicar a {bulkDocs.length} de la lista
            </button>
