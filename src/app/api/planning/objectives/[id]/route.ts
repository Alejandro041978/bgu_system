...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!body.revise) {
    ...
    const { data, error } = await (supabase as any)
      .from('strategic_objectives').update({ code: body.code }).eq('id', id).select().single()
  ...
  const { data: prev, error: prevErr } = await (supabase as any)
    .from('strategic_objectives').select('*').eq('id', id).single()
  ...
  const { data: next, error: insErr } = await (supabase as any)
    ...
    .insert({
      dimension_id: prev.dimension_id, code: prev.code,
  ...
  await (supabase as any).from('strategic_objectives')
...
export async function DELETE(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { error } = await (db() as any).from('strategic_objectives').delete().eq('id', id)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
