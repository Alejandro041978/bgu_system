...
import {
  categoriaElegible, esDelEquipo, estadoVisible, creditoDe,
  COSTO_DEGREE, CREDITO_POR_REFERIDO, type LeadMin,
} from '@/lib/referrals'
...
export async function POST(req: NextRequest) {
  ...
  // ── ¿Existe ya en el embudo comercial? ────────────────────────────────────
  const { data: leads } = await sb.from('sales_leads')
    .select('id, stage, last_contact_at, created_at, phone, phone_number, email')
  ...
  // ── El lead: se reutiliza el existente o nace uno nuevo en el embudo ──────
  ...
  if (status === 'registrado') {
    ...
    if (leadId) {
      await sb.from('sales_leads').update({
        notes: aviso, program_interest: prog?.name ?? null, updated_at: new Date().toISOString(),
      }).eq('id', leadId)
    } else {
