...
export default async function EmployeeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [empRes, contractsRes, instancesRes, facultyRes, yearsRes, credRes] = await Promise.all([
    ...
    (supabase as any).from('hr_employees').select('is_faculty, is_helpdesk, nacionalidad, first_names, last_names, faculty_email, faculty_email_sent_at, faculty_email_sent_to').eq('id', id).single(),
  ...
  const employee = { ...empRes.data, is_faculty: isFaculty, is_helpdesk: facultyRes.data?.is_helpdesk ?? false, nacionalidad: facultyRes.data?.nacionalidad ?? null, first_names: facultyRes.data?.first_names ?? null, last_names: facultyRes.data?.last_names ?? null, faculty_email: facultyRes.data?.faculty_email ?? null, faculty_email_sent_at: facultyRes.data?.faculty_email_sent_at ?? null, faculty_email_sent_to: facultyRes.data?.faculty_email_sent_to ?? null }
